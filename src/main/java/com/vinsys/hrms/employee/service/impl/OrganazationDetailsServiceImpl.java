...
public class OrganazationDetailsServiceImpl implements IOrganazationService {
	...
	@Autowired
	IEmployeeDepartmentDAO empDeptDAO;
		...
		if (!HRMSHelper.isLongZero(empId)) {
			...
			if (!HRMSHelper.isNullOrEmpty(employeeAddressBook)) {
				for (Employee employee : employeeAddressBook) {
					...
					employeeAddressVO.setBranchId(Optional.ofNullable(employee.getEmployeeBranch())
							.map(EmployeeBranch::getBranch).map(MasterBranch::getId).orElse(null));
	...
	public byte[] downloadEmployeeDetailsReport(String keyword) throws HRMSException {
		...
		try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			String[] headers = { "Employee ID", "Full Name", "Official Email", "Mobile", "Department", "Designation",
					"Branch", "Reporting Manager", "Grade", "Function" };
			...
			for (EmployeeMasterDetails emp : employees) {
				...
				cell1.setCellValue(
						HRMSHelper.safeString(emp.getFirstName()) + " " + HRMSHelper.safeString(emp.getLastName()));
	...
	public HRMSBaseResponse<List<HRListResponseVO>> getActiveHRList() throws HRMSException {
		...
		// Validate role - only HR can access
		if (!HRMSHelper.isRolePresent(roles, ERole.HR.name())) {
			throw new HRMSException(1500, ResponseCode.getResponseCodeMap().get(1521));
		}

		Long deptId = 172L; // HCD Department ID

		List<EmployeeDepartment> empDeptList = empDeptDAO.findByDepartmentId(deptId, ERecordStatus.Y.name());

		for (EmployeeDepartment empDept : empDeptList) {

			if (!HRMSHelper.isNullOrEmpty(empDept)) {

				HRListResponseVO vo = new HRListResponseVO();
				vo.setId(empDept.getEmployee().getId());
				vo.setOfficialEmailId(empDept.getEmployee().getOfficialEmailId());
				vo.setName(empDept.getEmployee().getCandidate().getFirstName() + " "
						+ empDept.getEmployee().getCandidate().getLastName());
				vo.setDepartment(
						Optional.ofNullable(empDept.getEmployee().getEmployeeDepartment())
								.map(EmployeeDepartment::getDepartment)
								.map(MasterDepartment::getDepartmentName)
								.orElse(null)
				);
				vo.setEmployeeCode(empDept.getEmployee().getEmployeeCode());
				vo.setGrade(
						Optional.ofNullable(empDept.getEmployee().getCandidate().getCandidateProfessionalDetail())
								.map(CandidateProfessionalDetail::getGrade)
								.orElse(null)
				);

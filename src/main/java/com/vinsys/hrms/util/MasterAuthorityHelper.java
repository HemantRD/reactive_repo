...
//		}
	...
	public void validateKpiQuestionVo(KpiQuestionRequestVO vo) throws HRMSException {

		if (vo == null) {
			throw new HRMSException(1501, "Invalid KPI Question Request");
		}
		...
		if (!HRMSHelper.regexMatcher(String.valueOf(vo.getCategoryId()), "[0-9]+")
				|| HRMSHelper.isLongZero(vo.getCategoryId())) {
		...
		if (!HRMSHelper.regexMatcher(String.valueOf(vo.getSubcategoryId()), "[0-9]+")
				|| HRMSHelper.isLongZero(vo.getSubcategoryId())) {

			throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " SubCategory Id ");
		}

		if (HRMSHelper.isNullOrEmpty(vo.getQuestionName())) {
			throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " Question ");
		}

		boolean questionNameValid = HRMSHelper.regexMatcher(vo.getQuestionName(),
				"^(?!\\s)(?=[\\s\\S]{1,250}$)[a-zA-Z0-9.,;:!?\"'\\s-]+(?<!\\s)$");

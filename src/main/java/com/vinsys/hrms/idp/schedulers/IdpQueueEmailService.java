...
public class IdpQueueEmailService {
    ...
    public void queueIdpReminderEmail() {
        ...
        List<IdpFlowHistoryEntity> pendingList = idpFlowHistoryDAO.getDataListForFollowupEmail();
        if (CollectionUtils.isEmpty(pendingList)) {
            return;
        }

        for (IdpFlowHistoryEntity entity : pendingList) {
            IdpEmailLogEntity saveEmailLog = new IdpEmailLogEntity();

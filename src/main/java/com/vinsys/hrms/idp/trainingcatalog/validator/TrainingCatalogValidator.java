...
public class TrainingCatalogValidator {
    ...
    public TrainingCatalogValidator(final TrainingCatalogDAO trainingCatalogDAO, final TrainingBudgetDAO trainingBudgetDAO,
                                    final IMasterYearDAO iMasterYearDAO) {
    ...
    public void validateAddUpdate(TrainingCatalogVo trainingCatalogVo) throws HRMSException {
        ...
        if (trainingCatalogVo.getDurationInHours() == null || trainingCatalogVo.getDurationInHours() <= 0) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " durationInHours");
        }
    ...
    public void validateAddUpdateBudget(TrainingBudgetVo trainingBudgetVo) throws HRMSException {
        if (trainingBudgetVo.getId() != null && !trainingBudgetDAO.existsById(trainingBudgetVo.getId())) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " Id");
        }
        if (HRMSHelper.isNullOrEmpty(trainingBudgetVo.getCurrencySymbol())) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " currencySymbol");
        }
         if (trainingBudgetVo.getYear() == null || trainingBudgetVo.getYear() <= 0 ||
                iMasterYearDAO.findByYear(trainingBudgetVo.getYear()) == null) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " year");
        }
        if (trainingBudgetVo.getBudgetAmount() == null || trainingBudgetVo.getBudgetAmount() <= 0) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " budgetAmount");
        }

        Long exitingId = trainingBudgetDAO.yearAlreadyExist(trainingBudgetVo.getYear(), trainingBudgetVo.getId());
        if (exitingId != null && exitingId > 0) {
            trainingBudgetVo.setId(exitingId);
        }
    }

    public void validateUpdateTrainingBudgetStatus(TrainingBudgetStatus trainingBudgetStatus) throws HRMSException {
        if (!trainingBudgetDAO.existsById(trainingBudgetStatus.getId())) {
            throw new HRMSException(1501, "Not Found");
        }
        if (HRMSHelper.isNullOrEmpty(trainingBudgetStatus.getStatus())) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " status");
        }
    }

    public void getAllTrainingCatalogs(TrainingCatalogListingReq request) throws HRMSException {
        if (HRMSHelper.isNullOrEmpty(request.getStatus())) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " status");
        }
        ...
        if (!HRMSHelper.isNullOrEmpty(request.getSortBy()) &&
                (!request.getSortBy().equals("trainingCode") && !request.getSortBy().equals("topicName")
                        && !request.getSortBy().equals("costPerPersonIndividual")
                        && !request.getSortBy().equals("costPerPersonGroup")
                        && !request.getSortBy().equals("costPerGroup")
                        && !request.getSortBy().equals("minPersonInGroup")
                        && !request.getSortBy().equals("maxPersonInGroup")
                        && !request.getSortBy().equals("priority")
                        && !request.getSortBy().equals("durationInHours"))) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) +
                    " sortBy (trainingCode/topicName/costPerPersonIndividual/costPerPersonGroup/costPerGroup/minPersonInGroup/maxPersonInGroup/priority/durationInHours)");
        }
        if (HRMSHelper.isNullOrEmpty(request.getSortBy())) {
    ...
    public void updateTrainingStatus(TrainingCatalogStatus trainingCatalogStatus) throws HRMSException {
        if (!trainingCatalogDAO.existsById(trainingCatalogStatus.getId())) {
            throw new HRMSException(1501, "Not Found");
        }
        if (HRMSHelper.isNullOrEmpty(trainingCatalogStatus.getStatus())) {
            throw new HRMSException(1501, ResponseCode.getResponseCodeMap().get(1501) + " status");
        }
    }

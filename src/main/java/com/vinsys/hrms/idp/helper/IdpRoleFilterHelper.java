...
//            return getHrIdps(loggedInEmpId);
    ...
    private List<Idp> getDivisionHeadIdps(Long divisionHeadId) {
        List<Long> divisionIds = funcationHeadToDivisionFunctionDAO
                .getDivisionByEmployeeAndIsActive(divisionHeadId, ERecordStatus.Y.name());

        if (HRMSHelper.isNullOrEmpty(divisionIds)) {
            log.info("No divisions found for DIVISIONHEAD employee: {}", divisionHeadId);
            return Collections.emptyList();
        }

        log.info("DIVISIONHEAD employee {} has access to divisions: {}", divisionHeadId, divisionIds);

        // Use view to fetch IDPs by division
        List<IdpListReportView> viewResults = idpListReportViewDAO.findByEmpDivisionIdIn(divisionIds);

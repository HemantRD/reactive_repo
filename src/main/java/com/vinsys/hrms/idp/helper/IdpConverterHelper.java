...
public class IdpConverterHelper {
    ...
    public List<com.vinsys.hrms.idp.vo.getidp.IdpListItemVO> convertIdpListToListItemVOs(
        ...
        return idpList.stream().map(idp -> {
            ...
            //com.vinsys.hrms.entity.Employee employee = employeeDAO.findActiveEmployeeById(idp.getEmployeeId(), com.vinsys.hrms.constants.ERecordStatus.Y.name());
            Employee employee = employeeDAO.findById(idp.getEmployeeId()).get();
            if (employee != null && employee.getCandidate() != null) {
                ...
                EmployeeReportingManager erm = employee.getEmployeeReportingManager();
                if (!HRMSHelper.isNullOrEmpty(erm)) {
                    Employee rm = erm.getReporingManager();
    ...
    public com.vinsys.hrms.idp.vo.getidp.IdpVO convertToIdpVO(Idp idp, String lineManager,
        ...
        List<com.vinsys.hrms.idp.vo.getidp.IdpDetailVO> idpDetailVOList = idpDetailList.stream()
                .map(detail -> {
                    ...
                    // Get training ID from training object if available
                    if (detail.getTraining() != null) {
                        detailVO.setTrainingId(detail.getTraining().getId());
                    }
                    detailVO.setTrainingName(detail.getTrainingName());
                    detailVO.setPriority(detail.getPriority());

...
import com.vinsys.hrms.idp.vo.response.GenericResponseVO;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
     ...
     * @return a {@link ResponseEntity} containing a {@link GenericResponseVO}
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        return ResponseEntity.badRequest()
                .body(GenericResponseVO.error("Validation failed"));
    }
     ...
     *         containing a {@link GenericResponseVO} with the exception message
     */
    @ExceptionHandler(Exception.class)
    public ResponseEntity<GenericResponseVO> handleAll(Exception ex) {
        ex.printStackTrace(); // Log for debugging; consider replacing with a logger
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(GenericResponseVO.error(ex.getMessage()));
    }
}
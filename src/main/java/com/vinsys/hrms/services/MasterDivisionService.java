...
@Hidden
@RestController
@RequestMapping(path = "/masterDivision")
...
public class MasterDivisionService {
	...
	@ResponseBody
	public String createMasterDivision(@RequestBody VOMasterDivision voMasterDivision) {

		MasterDivision masterDivisionEntity;
		Organization organizationEntity;
		String resultMesage = "";

		try {
			if (!HRMSHelper.isNullOrEmpty(voMasterDivision)
					&& !HRMSHelper.isNullOrEmpty(voMasterDivision.getOrganization().getId())) {

				masterDivisionEntity = masterDivisionDAO.findById(voMasterDivision.getId()).get();
				if (!HRMSHelper.isNullOrEmpty(masterDivisionEntity)) {
					/* update */
					// masterDesignationEntity =
					// HRMSRequestTranslator.translateToMasterBranchEntity(masterDesignationEntity,voMasterDesignation);
					masterDivisionEntity = HRMSRequestTranslator.translateToMasterDivisionEntity(masterDivisionEntity,
							voMasterDivision);
					resultMesage = IHRMSConstants.updatedsuccessMessage;
				} else {
					/* insert */
					masterDivisionEntity = new MasterDivision();
					organizationEntity = organizationDAO.findById(voMasterDivision.getOrganization().getId()).get();
					if (!HRMSHelper.isNullOrEmpty(organizationEntity)) {
						masterDivisionEntity.setOrgId(organizationEntity.getId());
						// masterDesignationEntity =
						// HRMSRequestTranslator.translateToMasterBranchEntity(masterDesignationEntity,voMasterDesignation);
						masterDivisionEntity = HRMSRequestTranslator
								.translateToMasterDivisionEntity(masterDivisionEntity, voMasterDivision);
					} else {
				...
				masterDivisionDAO.save(masterDivisionEntity);
	...
	@ResponseBody
	public String getAllMasterDivision(@PathVariable("id") long orgId) {

		List<MasterDivision> masterDivisionEntityList = new ArrayList<>();
		List<Object> voMasterDivisionList = new ArrayList<>();
		HRMSListResponseObject hrmsListResponseObject = new HRMSListResponseObject();
		try {
			masterDivisionEntityList = masterDivisionDAO.findAllMasterDivisionByOrgIdCustomQuery(orgId,
					IHRMSConstants.isActive);
			if (!HRMSHelper.isNullOrEmpty(masterDivisionEntityList)) {
				voMasterDivisionList = HRMSResponseTranslator.transalteToMasterDivisionVO(masterDivisionEntityList,
						voMasterDivisionList);
				hrmsListResponseObject.setListResponse((List<Object>) voMasterDivisionList);
	...
	@ResponseBody
	public String deleteMasterDivision(@PathVariable("id") long divisionId) {

		try {
			if (!HRMSHelper.isNullOrEmpty(divisionId)) {
				masterDivisionDAO.deleteById(divisionId);

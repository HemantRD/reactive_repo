...
@RequestMapping(path = "/masterDesignation")
...
public class MasterDesignationService {
	...
	public String createMasterDesignation(@RequestBody VOMasterDesignation voMasterDesignation) {
		...
		try {
			if (!HRMSHelper.isNullOrEmpty(voMasterDesignation)
					&& !HRMSHelper.isNullOrEmpty(voMasterDesignation.getOrganization().getId())) {
				...
				if (!HRMSHelper.isNullOrEmpty(masterDesignationEntity)) {
					...
					if (!HRMSHelper.isNullOrEmpty(organizationEntity)) {
						...
						// masterDesignationEntity =
						// HRMSRequestTranslator.translateToMasterBranchEntity(masterDesignationEntity,voMasterDesignation);
						masterDesignationEntity = HRMSRequestTranslator.translateToMasterDesignationEntity(masterDesignationEntity, voMasterDesignation);
					} else {
	...
	@ResponseBody
	public String getAllMasterdesignation(@PathVariable("id") long orgId) {
		...
		try {
			masterDesignationEntityList = masterDesignationDAO.findAllMasterDesignationByOrgIdCustomQuery(orgId,IHRMSConstants.isActive);
	...
	public String deleteMasterBranch(@PathVariable("id") long desigationId) {
		...
		try {
			if (!HRMSHelper.isNullOrEmpty(desigationId)) {
				masterDesignationDAO.deleteById(desigationId);

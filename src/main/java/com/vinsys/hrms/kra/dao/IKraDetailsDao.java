...
//	public EmployeeACN findByEmpIdAndIsActive(Long empId,String isActive);
	...
	@Query(value = "SELECT ROUND( "
			+ "CAST((SUM(tmkr.value * ttkd.objective_weightage) / NULLIF(SUM(ttkd.objective_weightage), 0)) AS numeric), "
			+ "2 " + ") AS avg " + "FROM tbl_trn_kra_details ttkd "
			+ "JOIN tbl_mst_kra_rating tmkr ON tmkr.id = ttkd.self_rating " + "WHERE ttkd.kra_id = ?1 "
			+ "AND ttkd.category_id = ?2 " + "AND ttkd.is_active = ?3 "
			+ "AND ttkd.weightage != ?4", nativeQuery = true)
	Double calculateSelfCategorywiseAverage(Long kraId, Long categoryId, String status, Float weightage);
	
	
	@Query(value = "SELECT ROUND( "
			+ "CAST((SUM(tmkr.value * ttkd.objective_weightage) / NULLIF(SUM(ttkd.objective_weightage), 0)) AS numeric), "
			+ "2 " + ") AS avg " + "FROM tbl_trn_kra_details ttkd "
			+ "JOIN tbl_mst_kra_rating tmkr ON tmkr.id = ttkd.manager_rating " + "WHERE ttkd.kra_id = ?1 "
			+ "AND ttkd.category_id = ?2 " + "AND ttkd.is_active = ?3 "
			+ "AND ttkd.weightage != ?4", nativeQuery = true)
	Double calculateCategorywiseManagerAverage(Long kraId, Long categoryId, String status, Float weightage);

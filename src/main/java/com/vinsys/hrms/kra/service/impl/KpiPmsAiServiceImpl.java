...
public class KpiPmsAiServiceImpl implements IKpiPmsAiService {
    ...
    private String pmsAiMsEndPointDepartmentalAnalysis;


    private KraCycle fetchActiveCycle() throws Exception {
        //Fetch active cycle
    ...
    public void calculateDepartmentLevelAggregations(AiMsBackgroundAnalysisRequestInvokerVO invokerVO, Long deptId) throws Exception {
        try {
            ...
            KraCycle cycle = fetchActiveCycle();
    ...
    public void doDepartmentLevelAnalysis(AiMsBackgroundAnalysisRequestInvokerVO invokerVO, Long deptId) throws Exception {
        try {
            ...
            for (Long dptId : departmentIds) {
                ...
                departmentDataVo.setDepartmentName(departmentDAO.findByIdAndIsActive(dptId, ERecordStatus.Y.name()).getDepartmentName());

                List<KraAggregateScores> deptLevelData;
                deptLevelData = kraAggregateScoresDao.findByKraCycleIdAndLevelOfAggregationAndDepartment_Id(cycle.getId(), "Department", dptId);
                ...
                for (KraDetailsReport kraDetailsReport : kraList) {
                    ...
                    if (!HRMSHelper.isNullOrEmpty(kra)) {
                        ...
                        if (!HRMSHelper.isNullOrEmpty(designation)) {
                            employeeDetailsVo.setDesignation(designation.getDesignation().getDesignationName());
                        }
                        employeeDetailsVo.setDivision(employee.getEmployeeDivision().getDivision().getDivisionName());
                        ...
                        Employee reportingManager = employeeDAO.findByIsActiveAndId(ERecordStatus.Y.name(), kraDetailsReport.getRmId());
                        reportingManagerDetailsVo.setName(kraDetailsReport.getReportingManager());
                        ...
                        if (!HRMSHelper.isNullOrEmpty(rmDesignation)) {
                            reportingManagerDetailsVo.setDesignation(rmDesignation.getDesignation().getDesignationName());
                        }
                        reportingManagerDetailsVo.setDivision(reportingManager.getEmployeeDivision().getDivision().getDivisionName());
                ...
                log.info("Completed data fetching for departmentId: " + dptId);
            }
            ...
            try {
                ...
                if (apiResult.getStatusCode() != HttpStatus.OK) {
                    log.error("Error in doDepartmentLevelAnalysis():E1: HTTP Status Code = " + apiResult.getStatusCode());
                    ...
                    log.debug("The response from PMS AIMS : \n\n" + apiResponseBody + "\n\n\n.");// Type = AiMsDepartmentLevelAnalysisResponseVO
                    ...
                    if (!HRMSHelper.isNullOrEmpty(pmsAiMsResponse)) {
                        ...
                        if (!HRMSHelper.isNullOrEmpty(pmsAiMsResponse.getDepartments())) {
                            ...
                            for (AiMsDepartmentLevelAnalysisResponseVO.DepartmentDataVo deptVo : pmsAiMsResponse.getDepartments()) {
                                List<KraAggregateScores> deptLevelData;
                                deptLevelData = kraAggregateScoresDao.findByKraCycleIdAndLevelOfAggregationAndDepartment_Id(cycle.getId(), "Department", deptVo.getId());
                        ...
                        log.error("Error in doDepartmentLevelAnalysis():E2: No Response body from AIMS : " + apiResult.getStatusCode());
                        throw new HRMSException(1500, "Error while calling AI service : No Response body : " + apiResult.getStatusCode());
                    }
                }

            } catch (Exception e) {
                log.error("Error in doDepartmentLevelAnalysis():E3:", e);
                e.printStackTrace();
            }
            ...
            log.error("Error in doDepartmentLevelAnalysis: ", e);
            throw e;
        }
    }
    ...
    private void populateOrganisationData(List<AggregateCalcVo> aggregateCalcVoList, KraCycle cycle) {
        ...
        List<KraAggregateScores> orgLevelData = kraAggregateScoresDao.findByKraCycleIdAndLevelOfAggregation(cycle.getId(), "Organisation");
    ...
    private void populateDepartmentData(List<AggregateCalcVo> aggregateCalcVoList, KraCycle cycle) {
        ...
        for (Long departmentId : departmentList) {
            List<KraAggregateScores> deptLevelData = kraAggregateScoresDao.findByKraCycleIdAndLevelOfAggregationAndDepartment_Id(cycle.getId(), "Department", departmentId);
            KraAggregateScores deptScore;
            if (HRMSHelper.isNullOrEmpty(deptLevelData)) {
                //Insert new record here
                deptScore = new KraAggregateScores();
    ...
    private void populateDivisionData(List<AggregateCalcVo> aggregateCalcVoList, KraCycle cycle) {
        ...
        for (Long departmentId : departmentList) {
            ...
            for (Long divisionId : divisionList) {
                List<KraAggregateScores> deptLevelData = kraAggregateScoresDao.findByKraCycleIdAndLevelOfAggregationAndDepartment_IdAndDivision_Id(cycle.getId(), "Division", departmentId, divisionId);
                ...
                if (HRMSHelper.isNullOrEmpty(deptLevelData)) {
                    ...
                    divisionScore.setLevelOfAggregation("Division");
    ...
    public void doOrganisationLevelAnalysis(AiMsBackgroundAnalysisRequestInvokerVO invokerVO) throws Exception {
        try {
            ...
            try {
                ...
                // check if request is successful
                if (apiResult.getStatusCode() != HttpStatus.OK) {
                    log.error("Error in doOrganisationLevelAnalysis():E1: HTTP Status Code = " + apiResult.getStatusCode());
                    throw new HRMSException(1500, "Error while calling AI service : " + apiResult.getStatusCode());
                } else {
                    String apiResponseBody = apiResult.getBody();
                    log.debug("The response from PMS AIMS : \n\n" + apiResponseBody + "\n\n\n.");
                    ...
                    if (!HRMSHelper.isNullOrEmpty(pmsAiMsResponse)) {
                        ...
                        if (!HRMSHelper.isNullOrEmpty(pmsAiMsResponse.getOverallSummary())) {
                            ...
                            orgLevelData = kraAggregateScoresDao.findByKraCycleIdAndLevelOfAggregation(cycle.getId(), "Organisation");
                            KraAggregateScores orgScore;
                            if (HRMSHelper.isNullOrEmpty(orgLevelData)) {
                                //Insert new record here
                                orgScore = new KraAggregateScores();
                        ...
                        log.error("Error in doOrganisationLevelAnalysis():E2: No Response body from AIMS : " + apiResult.getStatusCode());
                        throw new HRMSException(1500, "Error while calling AI service : No Response body : " + apiResult.getStatusCode());
                    }
                }

            } catch (Exception e) {
                log.error("Error in doOrganisationLevelAnalysis():E3:", e);
                e.printStackTrace();
            }
            ...
            log.error("Error in doOrganisationLevelAnalysis: ", e);
            throw e;
        }
    }
}
